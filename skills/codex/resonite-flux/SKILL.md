...
Keep ProtoFlux logic in .pg files. Confirm `rloop flux status --json`, the active ResoniteLink URL, and that `rloop doctor --json` reports a successful managed-data check/build probe through either the configured path or Flux-SDK auto-discovery.
