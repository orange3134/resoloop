...
After deployment, use `parentSlotId` as the deploy destination and `moduleSlotIdBefore` / `moduleSlotIdAfter` as the re-observed direct module child identities; re-inspect that child and each generated input/output reference target. Stop watch on cancellation. Do not reimplement compilation or generate ProtoFlux nodes directly—use Flux-SDK. Treat an untested Flux-SDK series reported by doctor as a compatibility warning that must be resolved before live deployment. Flux-SDK 1.9.0 can return `Invalid component type` after wiring some interface-typed global inputs; report it as a non-atomic upstream deployment failure, inspect the exact partial module, and do not mark it successful.
